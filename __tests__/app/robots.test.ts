...
describe("robots.ts", () => {
	...
	it("should allow all user agents", () => {
		...
		const hasWildcardRule = robotsConfig.rules.some(
			(rule) => rule.userAgent === "*" && rule.allow === "/"
		);
	...
	it("should include domain in sitemap URL", () => {
		...
		expect(robotsConfig.sitemap).toMatch(
			/(undefined|localhost|diet-clic|manon-chaillou|nutrition|dietetique-et-interventions)/
		);
	});
	...
	it("should have proper disallow rules", () => {
		...
		const hasDisallowRules = robotsConfig.rules.some(
			(rule) =>
				rule.disallow &&
