...
interface ContactHeaderProps {
	contactPromise: Promise<GetContactReturn | null>;
}
...
export function ContactHeader({ contactPromise }: ContactHeaderProps) {
				...
				<div>
					<div className="flex flex-wrap items-center gap-3">
						...
						<div className="flex items-center gap-2">
							<Badge
								...
								style={{
									backgroundColor: `${
										CONTACT_STATUS_COLORS[contact.status as ContactStatus]
									}20`,
									color: CONTACT_STATUS_COLORS[contact.status as ContactStatus],
									borderColor:
										CONTACT_STATUS_COLORS[contact.status as ContactStatus],
								}}
							>
								{CONTACT_STATUS_LABELS[contact.status as ContactStatus]}
							</Badge>
						</div>
					</div>
					...
					<div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2 text-sm text-muted-foreground">
						...
						<div className="flex items-center gap-1.5">
							...
							<span className="font-mono">{contact.id.substring(0, 8)}</span>
						</div>
					</div>
